jQuery(document).ready(function ($) {
    ...
    var Expo360obj = function (parameters) {
        ...
        this.imagesSrc = Array(), this.imagesBigSrc = Array(), this.hotspots = Array(), this.hotspotsButtons = Array(), this.hotspotsTooltips = Array(), this.$hotspotsSmall;
    ...
    Expo360obj.prototype = {
        _this: this, loadXml: function () {
            var obj = this;
            $.ajax({
                type: "GET",
                url: obj.xml, dataType: "xml", success: function (xml) {
                    obj.parseXml(xml)
                }
            })
            ...
            this.mouse_wheel_function =
                $(configuration).find("mouse_wheel_function").text();
            ...
            if ($(configuration).find("maxZoom").text() == "auto") this.maxZoom = "auto"; else this.maxZoom = parseFloat($(configuration).find("maxZoom").text(), 10);
            if ($(configuration).find("reverse").text() == "true") this.reverse = true;
            if ($(configuration).find("autoplay").text() ==
                "true") this.autoplay = true;
            if ($(configuration).find("grab_hand_cursor").text() == "true") this.grab_hand_cursor = true;
            this.autoplaySpeed = parseInt($(configuration).find("autoplaySpeed").text(), 10);
            if ($(configuration).find("include_tooltips").text() == "true") this.include_tooltips = true;
            if ($(configuration).find("include_zoom_window").text() == "true") this.include_zoom_window = true;
            var panel_xml = $(configuration).find("panel");
            this.panel_options.width = parseInt($(panel_xml).find("width").text(), 10);
            this.panel_options.height =
                parseInt($(panel_xml).find("height").text(), 10);
            ...
            this.panel_options.round_corners = $(panel_xml).find("round_corners").text() +
                "px";
            ...
            this.panel_options.buttons_height = parseInt($(panel_xml).find("buttons_height").text(),
                10);
            ...
            this.panel_options.zoom_subbuttons_distance = parseInt($(panel_xml).find("zoom_subbuttons_distance").text(),
                10);
            ...
            this.loading_options.background_pattern =
                $(loading_xml).find("background_pattern").text();
            ...
            $(controlsXml).find("control").each(function (index) {
                var $this = $(this);
                if ($this.text() == "hyperlink") _this.controlsExtras.push($this.attr("href")); else _this.controlsExtras.push("none");
                _this.controls.push($this.text())
            });
            var tooltips_texts_xml =
                $(configuration).find("tooltips_texts");
            ...
            this.tooltips_texts.zoom_slider =
                $(tooltips_texts_xml).find("zoom_slider").text();
            ...
            this.zoom_window_options.border = $(zoom_window_xml).find("border").text();
            this.zoom_window_options.background_color =
                $(zoom_window_xml).find("background_color").text();
            ...
            this.imagesPath =
                $(configuration).find("imagesPath").text();
            this.imagesBigPath = $(configuration).find("imagesBigPath").text();
            if (this.parameters.imagesPath != undefined) this.imagesPath = this.parameters.imagesPath;
            if (this.parameters.imagesBigPath != undefined) this.imagesBigPath = this.parameters.imagesBigPath;
            if (this.imagesBigPath == "none") this.focusable = false;
            ...
            $(imagesXml).find("image").each(function (index) {
                ...
                if ($this.attr("srcBig")) _this.imagesBigSrc.push($this.attr("srcBig")); else _this.imagesBigSrc.push($this.attr("src"));
                ...
                if (spots.length > 0) {
                    ...
                    $(spots).each(function (index) {
                        ...
                        if (obj.type == "link") obj.content =
                            $hotspot.find("content").text();
                        if (obj.type == "small") {
                            var contObj = Object();
                        ...
                        if (obj.type == "big") {
                            ...
                            contObj.closeOffsetX =
                                parseInt($(content).find("closeOffsetX").text(), 10);
                ...
                } else _this.hotspots.push(null)
            });
            if (nImages == 1) this.singleImage = true;
            this.makeView()
        }, makeView: function () {
            ...
            this.$bigContent.css({"position": "absolute"});
            ...
            if (this.include_zoom_window) {
                ...
                this.$zoomClickable.css({"width": "100%", "height": "100%", "position": "absolute"});
                ...
                $filler.css({"width": "100%", "height": "100%", "background-color": "#ffffff", "opacity": "0"});
                this.$zoomWindowBox.append($filler);
                this.$zoomWindow.append(this.$zoomClickable, this.$zoomWindowBox);
                processBorder(this.$zoomWindow, this.zoom_window_options.border)
            }
            this.$view.css({
                "position": "relative",
                "width": "100%", "height": this.height + "px", "overflow": "hidden"
            });
            this.$panel.css({
                "position": "absolute",
            ...
            addRoundCorners(this.$panel,
                this.panel_options.round_corners);
            this.$loader.css({
                "zoom": "1",
                "position": "absolute",
                "width": "100%",
            ...
            this.$loadingText.css({
                ...
                "background-image": "url(/3d_viewer/ui_images/diada_loader.png)",
                "background-position": "0 0",
                "width": "315px",
                "height": "82px"
            });
            processColorAndPattern(this.$loader, this.loading_options.background_color, this.loading_options.background_alpha,
                this.loading_options.background_pattern);
            for (var i = 0; i < this.controls.length; i++) {
                if (this.controls[i] == "left" || this.controls[i] == "right" || this.controls[i] == "rotate" || this.controls[i] == "pan" || this.controls[i] == "hyperlink" || this.controls[i] == "reset" || this.controls[i] == "zoom-in" || this.controls[i] == "zoom-out") {
                    var $btn = $('<a href="#"></a>').appendTo(this.$panel);
                    $btn.css({
                        "position": "relative",
                        "float": "left",
                        "margin": this.panel_options.height / 2 - this.panel_options.buttons_height / 2 + "px " + this.panel_options.buttons_side_margin +
                        "px",
                    ...
                    if (this.include_tooltips) {
                        ...
                        var $tooltip = this.makeTooltip(str,
                            $btn, i)
                    }
                    if (this.controls[i] == "left") $btn.bind(mouseDownBind, $.proxy(this.leftControlDown, this)); else if (this.controls[i] == "right") $btn.bind(mouseDownBind, $.proxy(this.rightControlDown, this)); else if (this.controls[i] == "rotate") {
                    ...
                    } else if (this.controls[i] == "reset") $btn.bind(clickBind, $.proxy(this.resetControlClick, this));
                    else if (this.controls[i] == "zoom-out") $btn.bind(clickBind, $.proxy(this.zoomoutControlClick, this)); else if (this.controls[i] == "zoom-in") $btn.bind(clickBind, $.proxy(this.zoominControlClick, this)); else if (this.controls[i] == "hyperlink") {
                    ...
                    $minus.css({
                        ...
                        "margin": this.panel_options.height /
                        2 - this.panel_options.zoom_subbuttons_height / 2 + "px 0px",
                    ...
                    $zoom_slider.css({"width": this.sliderWidth});
                    ...
                    $plus.bind(mouseDownBind,
                        $.proxy(this.zoomPlusControlDown, this));
                    ...
                    var playback_slider_height = Math.max(this.panel_options.slider_height,
                        this.panel_options.dragger_height);
                    $playback_slider.css({
                        "position": "relative",
                        "float": "left",
                    ...
                    addRoundCorners($back, this.panel_options.slider_round_corners);
                    processColorAndPattern($back, this.panel_options.slider_background_color, this.panel_options.slider_background_alpha, this.panel_options.slider_background_pattern);
                    $slider.css({"position": "absolute", "left": -this.panel_options.dragger_width / 2 + "px"});
                    $playback_slider.css({"width": this.playbackSliderWidth});
                    $playback_slider.append($back,
                        $slider);
                    ...
                    var $play = $('<a href="#"></a>');
                    buttonsClass($play, this.panel_options.ui_folder + "play_out.png", this.panel_options.ui_folder + "play_over.png", this.panel_options.buttons_width,
                        this.panel_options.buttons_height, this.panel_options.buttons_tween_time);
                    var $pause = $('<a href="#"></a>');
                    ...
                    $play.css({"position": "absolute", "top": "0", "left": "0"});
                    $pause.css({"position": "absolute", "top": "0", "left": "0"});
                    $btn.append($play, $pause);
                    this.$panel.append($btn);
                    this.$autoplayPlay =
                        $play;
                    this.$autoplayPause = $pause;
                    if (this.autoplay) $play.stop().fadeTo(0, 0).css("display", "none"); else $pause.stop().fadeTo(0, 0).css("display", "none");
                    $btn.click($.proxy(this.autoplayClick, this));
                    if (this.include_tooltips) {
                        var str = this.tooltips_texts.play;
                        if (this.autoplay) str = this.tooltips_texts.pause;
                ...
                if (i != this.controls.length - 1) {
                    ...
                    $sep.css({
                        "position": "relative",
                        "float": "left",
                        "margin": this.panel_options.height / 2 - this.panel_options.divider_height /
                        2 + "px 0px",
            ...
            if (this.mouse_wheel_function != "none") this.$view.mousewheel($.proxy(this.mouseWheelControl, this));
            ...
            if (this.panel_options.show == "roll_over" && !ismobile) {
                ...
                this.$main.hover(function () {
                    if (!_this.$panel.hasClass("blocked")) _this.$panel.stop().fadeTo(500, 1)
                }, function () {
            ...
            if (this.autoplay) this.speedDegDefault =
                this.autoplaySpeed; else this.speedDegDefault = 0;
            ...
            this.zoomBtnInterval = setInterval($.proxy(this.incrementZoom,
                this), 100);
            ...
            if (pos < this.sliderFrom) pos = this.sliderFrom;
            if (pos > this.sliderTo) pos = this.sliderTo;
            this.$zoomSlider.css({"left": pos + "px"});
            var perc = (pos + this.panel_options.dragger_width /
                2) / this.sliderWidth;
            ...
            if (pos < this.sliderFrom) pos = this.sliderFrom;
            if (pos > this.sliderTo) pos = this.sliderTo;
            this.$playbackSlider.css({"left": pos + "px"});
            var perc = (pos + this.panel_options.dragger_width /
                2) / this.playbackSliderWidth;
            this.goToDegree = true;
        ...
        playbackBackClick: function (e) {
            ...
            if (this.autoplay) {
                ...
                this.$autoplayPause.stop().fadeTo(this.panel_options.buttons_tween_time, 0, function () {
                    _this.$autoplayPause.css("display", "none")
                })
            } else {
                ...
                this.$autoplayPlay.stop().fadeTo(this.panel_options.buttons_tween_time,
                    0, function () {
                        _this.$autoplayPlay.css("display", "none")
                    })
            }
            ...
            this.clearSmall();
            if (this.mouse_wheel_function == "zoom") this.zoom += delta * this.mouse_wheel_speed; else if (this.mouse_wheel_function == "rotate") this.speedDeg += delta * this.mouse_wheel_speed;
            return false
            ...
            img.onload = function () {
                if (num == 0) {
                    ...
                    var imgH = img.height;
                    if (!(imgW ==
                        _this.width && imgH == _this.height)) {
                        var ratio = imgW / _this.width;
                        if (ratio > imgH / _this.height) ratio = imgH / _this.height;
                    ...
                    _this.$imagesHolder.css({"width": _this.iniWidth + "px", "height": _this.iniHeight + "px"});
                    if (_this.include_zoom_window) {
                        var ratio = 1;
                        if (_this.zoom_window_options.window_width == "auto") ratio =
                            imgH / _this.zoom_window_options.window_height; else if (_this.zoom_window_options.window_height == "auto") ratio = imgW / _this.zoom_window_options.window_width; else {
                        ...
                        _this.$zoomWindow.css({"width": w1 + "px", "height": h1 + "px"});
                ...
                if (_this.include_zoom_window) {
                    var $small = $image.clone().css({
                        ...
                        "width": "auto",
                        "height": "auto"
                    }).prependTo(_this.$zoomWindow);
                    _this.zoom_window_images.push($small)
                }
                _this.images.push($image);
                $image.bind(mouseDownBind, $.proxy(_this.clearSmall, _this));
                if (num == _this.imagesSrc.length - 1) _this.loadFinished(); else _this.loadImage(++num)
            };
            per_cent =
                100 / _this.imagesSrc.length * (num + 1);
            if (per_cent > 99) bg_pos = "0 -656px"; else if (per_cent > 93) bg_pos = "0 -574px"; else if (per_cent > 75) bg_pos = "0 -492px"; else if (per_cent > 60) bg_pos = "0 -410px"; else if (per_cent > 45) bg_pos = "0 -328px"; else if (per_cent > 30) bg_pos = "0 -246px"; else if (per_cent > 15) bg_pos = "0 -164px"; else if (per_cent > 7) bg_pos = "0 -82px"; else bg_pos = "0 0";
            this.$loadingText.css({"background-position": bg_pos});
            ...
            if (ismobile)if (ignored != e.pageX && ignore2 > 2) this.currentX = e.pageX; else {
                if (ignore2 <= 2) {
                    ignored = e.pageX;
                    ignore2++
                }
            } else this.currentX = e.pageX
        },
        onRotateUp: function () {
            ...
            var x;
            var y;
            if (ismobile)if (ignored != e.pageX && ignored1 != e.pageY && ignore2 > 2) {
                x = e.pageX;
                y = e.pageY
            } else {
                if (ignore2 <= 2) {
            ...
            var excessHeight =
                (this.currentHeight - this.height) / 2;
            if (toX < -excessWidth) toX = -excessWidth;
            if (toX > excessWidth) toX = excessWidth;
            if (toY < -excessHeight) toY = -excessHeight;
            if (toY > excessHeight) toY = excessHeight;
            this.viewLeft = toX;
            this.viewTop = toY
        }, onPanUp: function () {
        ...
        dragAndDrop: function () {
            ...
            if (this.include_zoom_window) {
                ...
                _this.$zoomWindowBox.bind(mouseDownBind, function (e) {
                    ...
                    $(document).bind(mouseMoveBind, function (e) {
                        var x = e.pageX;
                        var y = e.pageY;
                        var toX = prevLeft - (positionX - x) * (_this.width /
                            parseInt(_this.$zoomWindowBox.css("width"), 10));
                    ...
                    $(document).bind(mouseUpBind, function (e) {
                        _this.grabHandOpen(_this.$imagesHolder,
                            _this);
                        _this.panning = false;
                        unbindMoveAndUp();
                        return false
            ...
            $tooltip.css({
                ...
                "padding": this.tooltips_options.top_bottom_padding + "px " + this.tooltips_options.left_right_padding +
                "px",
            ...
            function over() {
                if (!$(this).hasClass("disabled")) {
                    ...
                    if (isIE) $triangle.stop().fadeTo(_this.tooltips_options.fadeTime,
                        _this.tooltips_options.background_alpha)
                }
            }
            ...
            function out() {
                if (!$(this).hasClass("removing")) {
                    ...
                    if (isIE) $triangle.stop().fadeTo(_this.tooltips_options.fadeTime, 0)
                }
            }
            ...
            if ($objArray != null) $($objArray).each(function () {
        ...
        getHint: function (control) {
            ...
            switch (control) {
                ...
                case "zoom-out":
                    ...
                    break;
                case "zoom_slider":
                    var perc = Math.round((this.zoom -
                            this.minZoom) / (this.maxZoom - this.minZoom) * 100) + "";
                ...
                case "rotate_slider":
                    ...
                    if (this.degrees < 0) degreesCalc = 360 + this.degrees;
                    if (this.draggingPlaybackSlider) deg = Math.round(this.goToDegreeNum) + ""; else deg = Math.round(degreesCalc) + "";
            ...
            if (this.$hotspotsSmall != null) {
                this.$hotspotsSmall.stop().fadeTo(this.smallTweenTime,
                    0, function () {
                        $(this).remove()
                    });
            ...
            if (this.$hotspotsSmall != null) this.$hotspotsSmall.css({
            ...
            var _this = this;
            var num = -1;
            for (var i = 0; i < this.hotspotsButtons.length; i++)if (this.hotspotsButtons[i].id == hotspot.id) {
                num = i;
                break
            }
            if (num == -1)return null;
            var w = parseInt(this.hotspotsButtons[num].width,
                10);
            ...
            if (hotspot.tooltip != "none" && hotspot.tooltip != null) {
                ...
                var $tooltip = this.makeTooltip(str, $btn, "cont" +
                    rel);
                this.currentHotspotsTooltips.push($tooltip)
            }
            if (hotspot.type == "link") {
                $btn.attr({"href": hotspot.content, "target": "_blank"});
                if (ismobile) $btn.bind(clickBind, function () {
                    window.location = hotspot.content;
                    return false
                })
            }
            if (hotspot.type == "small") $btn.bind(clickBind, function () {
                _this.clearSmall();
                ...
                $("a", $small_content).live(mouseDownBind, function () {
                    return true
                });
                $("a", $small_content).live(mouseUpBind, function () {
                    return true
                });
                $("a", $small_content).live(clickBind, function () {
                    return true
                });
                return false
            }); else if (hotspot.type == "big") $btn.bind(clickBind, function makeBigHotspotContent() {
                var contentObj = hotspot.content;
                var $hotspot = $(this);
                var $iframe;
                ...
                _this.$bigContent.css({"width": "100%", "height": "100%", "opacity": "0"});
                ...
                var numClose = -1;
                for (var i = 0; i < _this.hotspotsButtons.length; i++)if (_this.hotspotsButtons[i].id == contentObj.close_button_id) {
                    numClose = i;
                    break
                }
                if (numClose == -1)return null;
                var wClose = parseInt(_this.hotspotsButtons[numClose].width, 10);
                var hClose = parseInt(_this.hotspotsButtons[numClose].height,
                    10);
                ...
                $closeBtn.click(function () {
                    if (_this.panel_options.show ==
                        "roll_over") _this.$panel.removeClass("blocked").stop().fadeTo(500, 1);
                    _this.$bigContent.stop().fadeTo(contentObj.fadeTime, 0, function () {
                        $(this).empty().css({"width": "0", "height": "0"})
                    });
                    return false
                });
                if (_this.panel_options.show == "roll_over") _this.$panel.addClass("blocked").stop().fadeTo(500, 0);
                return false
            });
            return $btn
        }, clearHotspots: function () {
            for (var i = 0; i < this.currentHotspots.length; i++)this.currentHotspots[i].addClass("disabled removing").stop().fadeTo(500, 0, function () {
                $(this).remove()
            });
            for (var i =
                0; i < this.currentHotspotsTooltips.length; i++)this.currentHotspotsTooltips[i].stop().fadeTo(500, 0, function () {
                $(this).remove()
            });
            this.currentHotspots = Array();
            this.currentHotspotsTooltips = Array()
        }, updateHotspots: function () {
            for (var i = 0; i < this.currentHotspots.length; i++)if (this.hotspots[this.currentImage] != null) {
                ...
                updateTooltipPosition(this.currentHotspots[i],
                    "cont" + this.currentHotspots[i].attr("rel"))
            }
            this.updateSmall()
        }, bringHotspotsTimeout: function () {
            if (this.hotspots[this.currentImage] != null)for (var i = 0; i < this.hotspots[this.currentImage].length; i++) {
                var hotspot = this.hotspots[this.currentImage][i];
            ...
            if (this.hotspots[this.currentImage] != null) this.hotspotsTimer = setTimeout($.proxy(this.bringHotspotsTimeout, this), 100)
        }, getToFront: function () {
            if (this.currentImage >= 0 && this.currentImage < this.numImages)if (this.currentImage != this.oldImage) {
                for (var i = 0; i < this.numImages; i++) {
                    ...
                    if (this.include_zoom_window) this.zoom_window_images[i].css("display", "none")
                }
                this.images[this.currentImage].css("display", "block");
                if (this.include_zoom_window) this.zoom_window_images[this.currentImage].css("display",
                    "block");
            ...
            this.focused = false;
            this.focusedNum = -1;
            if (this.$bigImage != null) this.$bigImage.remove()
        }, changeFocus: function () {
            if (this.focusable) {
                this.removeFocus();
                if (this.zoom > 1 && !this.dragging) this.focusTimer = setTimeout($.proxy(this.updateFocus, this), 500)
            }
            ...
            if (!this.dragging && this.focusable) {
                ...
                img.onload = function () {
                    if (num == _this.currentImage && _this.zoom > 1) {
                        if (_this.focused) _this.removeFocus();
                        _this.focused = true;
                        _this.focusing = false;
                        _this.focusedNum = num;
                        var $image_holder = _this.images[num];
                        var $img = $(img);
                        $img.css({"top": "0", "left": "0", "width": "100%", "height": "100%", "position": "absolute"});
                        $image_holder.append($img);
                        _this.$bigImage = $img
                    }
                };
                img.src = this.imagesBigPath + this.imagesBigSrc[num]
            }
            ...
            if (!this.dragging && !this.goToDegree && !this.draggingPlaybackSlider) {
                ...
                var degreeEase;
                if (this.draggingPlaybackSlider) degreeEase = this.goToDegreeNum - this.degrees; else degreeEase = (this.goToDegreeNum - this.degrees) / this.ease;
                this.degrees += degreeEase;
                if (Math.round(this.degrees) ==
                    this.goToDegreeNum) {
            ...
            if (this.reverse)if (this.currentImage != 0) this.currentImage = this.numImages - this.currentImage;
            this.getToFront();
            ...
            this.currentWidth = Math.round(this.iniWidth *
                this.zoomCurrent);
            ...
            var viewTopCurrentAdd =
                (this.viewTop - this.viewTopCurrent) / this.paddingEase;
            ...
            if ((this.focused || this.focusing) && this.zoom <=
                1) this.removeFocus(); else if (!this.focusing && !this.focused && this.zoom > 1) this.updateFocus();
            ...
            if (this.include_zoom_window) this.$zoomWindowBox.css({
                ...
                "top": Math.round(-toY * (this.zoom_window_height / this.currentHeight)) +
                "px",
            ...
            if (this.$zoomSlider != null) {
                ...
                if (!this.draggingZoomSlider) {
                    ...
                    this.$zoomSlider.css({"left": pos + "px"});
                    if (this.zoom == this.minZoom) disableButton(this.$zoomMinus, this.buttonsDisabledAlpha);
                    else enableButton(this.$zoomMinus, this.buttonsDisabledAlpha);
                    if (this.zoom == this.maxZoom) disableButton(this.$zoomPlus, this.buttonsDisabledAlpha); else enableButton(this.$zoomPlus, this.buttonsDisabledAlpha)
                }
            }
        }, updateImage: function () {
            if (!this.singleImage) this.calculateDegree(); else if (!this.showedHotspots) {
                this.bringHotspots();
                this.showedHotspots = true
            }
            if (this.minZoom != this.maxZoom)if (Math.round(this.zoom * 1E3) != Math.round(this.zoomCurrent * 100)) this.calculateZoom();
            this.updateHotspots();
            if (this.dragging) {
                var dif =
                    this.currentX - this.positionClickedX;
            ...
            if (w > this.width || h > this.height) panable = true;
            if (this.include_zoom_window)if (panable && !this.zoomWindowEnabled) {
                this.$zoomWindow.css("display", "block").stop().fadeTo(500, 1);
                this.zoomWindowEnabled =
                    true
            } else if (!panable && this.zoomWindowEnabled) {
                this.$zoomWindow.stop().fadeTo(500, 0, function () {
                    $(this).css("display", "none")
                });
                this.zoomWindowEnabled = false
            }
            if (!panable && this.mode == "pan") this.mode = "rotate";
            if (this.$panButton != null)if (this.mode == "pan" || !panable) disableButton(this.$panButton, this.buttonsDisabledAlpha); else enableButton(this.$panButton, this.buttonsDisabledAlpha);
            if (this.$rotateButton != null)if (this.mode == "rotate") disableButton(this.$rotateButton, this.buttonsDisabledAlpha); else enableButton(this.$rotateButton,
                this.buttonsDisabledAlpha);
            setTimeout($.proxy(this.updateImage, this), refreshRate)
        }, grabHandOpen: function ($obj, _this) {
            if (_this.grab_hand_cursor) $obj.css("cursor", "url(" + _this.panel_options.ui_folder + "grabhand_open.png) 12 12,auto"); else $obj.css("cursor", "pointer")
        }, grabHandClose: function ($obj, _this) {
            if (_this.grab_hand_cursor) $obj.css("cursor", "url(" + _this.panel_options.ui_folder + "grabhand_closed.png) 12 12,auto"); else $obj.css("cursor", "pointer")
            ...
            $ele.bind(mouseDownBind, function initScrollDown(e) {
                ...
                $(document).bind(mouseMoveBind, function initScrollMove(e) {
                    ...
                    if (toX > 0) toX = 0; else if (toX < initwinWidth - eleWidth) toX = initwinWidth - eleWidth;
                    if (toY > 0) toY = 0; else if (toY < initwinHeight - eleHeight) toY = initwinHeight - eleHeight;
                    $initScrollObj.css({"left": toX, "top": toY});
                    return false
                });
    ...
    function processFont(object, font, color, size) {
        object.css({"font-family": font, "color": color, "font-size": size + "px"})
    }
    ...
    function processBorder(object, border) {
        object.css({"border": border})
    }

    function processColorAndPattern(object,
                                    color, alpha, pattern) {
        if (alpha != "0" && alpha != 0) {
            var filter = getFilter(color, alpha);
            var rgba = getRGBA(color, alpha);
            object.css({"background-color": color, "filter": filter, "background": rgba})
        }
        if (pattern != "none") object.css({"background-image": "url(" + pattern + ")", "background-repeat": "repeat"})
    }

    function preventDragDefault($obj) {
        if (isIE) $obj.get(0).onselectstart = function () {
            return false
        };
    ...
    function getFilter(color, alpha) {
        var color_alpha = parseInt(parseFloat(alpha,
                10) * 255).toString(16);
        var filter = "progid:DXImageTransform.Microsoft.gradient(startColorstr=#" + color_alpha + color.substring(1, 3) + color.substring(3, 5) + color.substring(5, 7) + ",endColorstr=#" + color_alpha + color.substring(1, 3) + color.substring(3, 5) + color.substring(5, 7) + ")";
        return filter
    }
