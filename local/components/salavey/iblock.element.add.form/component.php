		...
		foreach ($arParams["PROPERTY_CODES"]  as $i => $propertyID)
		{
			...
			if (intval($propertyID) > 0)
			{
				...
				else
				{
					...
					foreach ($arPropertyValue as $key => $value)
					{
                        if(is_array($_FILES["PROPERTY_FILE_".$propertyID."_".$key]['name']))
                        {
                            foreach($_FILES["PROPERTY_FILE_".$propertyID."_".$key]  as $param => $value)
                            {
                                foreach($value as $code => $val)
                                {
                                    $files[$code][$param] = $val;    
                                } 
                            }
                            
                            foreach($files as $fKey => $fParam)
                            {
                                $arFile = $fParam;
                                $arFile["del"] = $_REQUEST["DELETE_FILE"][$propertyID][$key] == "Y" ? "Y" : "";
                                $arUpdatePropertyValues[$propertyID][$fKey] = $arFile;
