...
<?if($arResult["OK_MESSAGE"] && $arResult['ORDER_ID']):?>
	<script type="text/javascript">
		yaCounter25448447.reachGoal('klik-ok');
		ga('send', 'pageview', '/klik-ok');
		console.log('Заказ в 1 клик отправлен на карточке и на странице корзины');
	</script>
	<div class="mf-ok-text" style="margin-top: 10px;"><?=$arResult["OK_MESSAGE"]?></div>
	...
	campaign_code=78f9a29420&action_code=1&response_type=img&uid=<?=$_COOKIE['admitad_uid']?>&order_id=<?=$arResult['ORDER_ID']?>&position_id=<?=($i+1)?>&tariff_code=<?=$arElement['PROPERTIES']['TARIFF_CODE']['VALUE']?>&currency_code=RUB&position_count=<?=count($arr)?>&price=<?=$arBasket['price']?>&quantity=<?=$arBasket['quantity']?>&product_id=<?=$arBasket['id']?>&coupon=&promocode =&payment_type=sale" width="1" height="1" alt="">
		<?endforeach;?>
	</noscript>

	<script type="text/javascript">
	    window.ad_order = "<?=$_REQUEST['order_id']?>";    // required
...
<?else:?>
    <form action="<?=POST_FORM_ACTION_URI?>" method="POST" id="ask_a_question" class="new_call_back_light dasket_one_click_popup one_click_form_block one_click_popup" style="padding: 0">
    <input type="hidden" name="PRODUCT_ID" value="<?=$arResult['PRODUCT_ID']?>">
    <input type="hidden" name="QUANTITY" value="<?=$arResult['QUANTITY']?>">
    <?=bitrix_sessid_post()?>

        <div class="one_click_form_title">Купить в 1 клик</div>
        ...
        <?if($arParams["SHOW_EMAIL"] == "Y"):?>
            <div class="one_click_form_el">
                <input type="text" name="user_email" class="required<?if(in_array('EMAIL',$arResult['ERROR_MESSAGE'])):?> sal_error<?endif?>" style="font-size: 20px;" placeholder="E-mail" value="<?=$arResult["AUTHOR_MAIL"]?>"/>
            </div>
        <?endif;?>
        <div class="one_click_form_el">
            <input type="text" class="required<?if(in_array('PHONE',$arResult['ERROR_MESSAGE'])):?> sal_error<?endif?>" name="user_phone" id="one_click_user_phone" style="font-size: 20px;" placeholder="Телефон" value="<?=$arResult["PHONE"]?>"/>
            <script>
                maskAktive("",true,"#one_click_user_phone"); //- закомментил в связи с задачей № 14781
