...
//    echo '<pre>'; echo '<br>'; var_export($arItem); echo '</pre>';
    ...
    <div class="newcat_el_bl<?=$arResult['SHOW_INFO'] ? ' admincat' : ''?>" id="<?=$strMainID;?>">
        <?php if ( !empty($resizeImg) ): ?>
        <script type="application/ld+json">
        {
            "@context": "https://schema.org",
            "@type": "ImageObject",
            "contentUrl": "<?=$resizeImg?>",
            ...
            <div class="newcat_ic_bl">
                <a href="<?=$arItem['DETAIL_PAGE_URL']?>">
                    ...
                    if(!empty($arItem['SECOND_PICTURE']))
                    {
                        ?>
                        <span class="one">
                            <img src="<?=$resizeImg?>" alt="<?=$arItem['NAME']?>">
                        </span>
                        <span class="two">
                            <img src="<?=$arItem['SECOND_PICTURE']['src']?>" alt="<?=$arItem['NAME']?>">
                        </span>
                        <?    
                    }
                    else
            ...
            <div class="newcat_info_bot">
				...
				<?if ( $indikator !== 'net' ):?>		
					<div id="buy_element_button_block_<?=$arItem["ID"]?>">
						<a itemscope itemtype="http://schema.org/BuyAction"  class="buy_element_button newcat_buy fancy_pay_war" data-price="" data-name="<?=$arItem['NAME']?>" data-id="<?=$arItem['ID']?>" data-set="<?=!empty($arItem['CUSTOM_PRICE']) ? 1 : ''?>" href="<?=$arItem['ADD_URL']?>">Купить</a>
					</div>
				<?endif;?>
            </div>
            <?
...
if ($paramValue < $pageCount) {
    ...
    echo sprintf('<div class="ajax_pager_wrap_block"><div class="ajax-pager-wrap">
                      <a class="ajax-pager-link index_more_produce" data-wrapper-class="newcat_list" href="%s">показать еще %s</a>
