	...
	BX.Sale.OrderAjaxComponent = {
		...
		bindMyEvents: function()
		{
			BX.bindDelegate(
				...
				'click',
				{tag:'input','name':'DELIVER_ID'},
				BX.proxy(this.ShowBoxBerryMap3, this)
			);
			
			
		},
		
		ShowBoxBerryMap3: function()
		{
			var currentDelivery = this.getSelectedDelivery();
			if(currentDelivery.ID == 204 && !this.bAlreadyShowBoxBerryMap)
			{
		...
		ShowBoxBerryMap2: function(text)
		{
			if(!this.bAlreadyShowBoxBerryMap)
			{
				this.bAlreadyShowBoxBerryMap = true;
				var re = /^.*?onclick="(.*?)".*?$/;
				var code = text.replace(re, '$1').replace('return false;','');
		...
		{
			var region = ' МОСКВА ';			
			var _locationNode = this.orderBlockNode.querySelector('div[data-property-id-row="17"');
			
			if(_locationNode)
			{
				var _loc = _locationNode.querySelector('input.bx-ui-sls-fake[type=text]');
				if(_loc)
				{
					region = ' '+(_loc.getAttribute('title').split(',',1)[0]).toUpperCase()+' ';
					
				}
			}
			
			boxberry.checkLocation(1);
			boxberry.open(delivery, '1$vUXV7BDt8pxTBD0HMCCoMblmtgcbbh55' , region , '', '100' , '500' ,'100' ,'0','0','0' ); 
			return false;
		},
