...
if ($arParams['USE_CUSTOM_MAIN_MESSAGES'] != 'Y')
{
    ...
    $arParams['MESS_TO_BASCET'] = Loc::getMessage('MESS_TO_BASCET');
...
$this->addExternalJs('//api-maps.yandex.ru/2.1.34/?load=package.full&lang='.$locale);
?>
	<NOSCRIPT>
...
else
{
	...
	?>
    <?if (preg_match("/personal\/cart/",$_SERVER["HTTP_REFERER"])):?>
        <script>
            $(document).ready(function ($) {
                if ($("#bx-soa-basket .pull-right").length>0) {
                    var event = new Event("click");
                    $("#bx-soa-basket .pull-right")[0].dispatchEvent(event);
                }
            })
        </script>
    <?endif;?>
	<form action="<?=$APPLICATION->GetCurPage();?>" method="POST" name="ORDER_FORM" id="bx-soa-order-form" class="standart_width" enctype="multipart/form-data">
		<?
		...
		<div id="bx-soa-order" class="row bx-<?=$arParams['TEMPLATE_THEME']?>" style="opacity: 0">
			...
			<ul class="basket_page_navigation_block not_style">
                <li><a href="/personal/cart/"><div class="basket_page_navigation_el icon1 current">Ваша корзина</div></a></li>
				<li><div class="basket_page_navigation_el icon2 active">Детали получения</div></li>
			...
			<div class="col-sm-9 bx-soa">
				...
				<? if ($arParams['DELIVERY_TO_PAYSYSTEM'] === 'p2d'): ?>
					...
					<div id="bx-soa-delivery" data-visited="false" class="bx-soa-section bx-active" <?=($hideDelivery ? 'style="display:none"' : '')?>>
						...
						<!--<div class="order_red_txt">Стоимость доставки может отличаться в зависимости от габаритов и веса груза, она будет рассчитана менеджером при подтверждении заказа.</div>
						<div class="bx-soa-section-content container-fluid"></div>-->
					</div>
					<!--	PICKUP BLOCK	-->
				...
				<div id="bx-soa-orderSave" class="hidden-xs">
                    ...
                    <p class="personaldata">Нажимая кнопку "<b>ОФОРМИТЬ ЗАКАЗ</b>", я подтверждаю свою дееспособность, согласие на получение информации об оформлении и получении заказа, согласие на обработку персональных данных в соответствии с указанным <a href="javascript:void(0)" class="showuserconset" style="color: #a41618"><b>здесь</b></a> текстом. Также подтверждаю, что мне исполнилось 18 лет.</p>
				</div>
	...
	{
		//file_put_contents($_SERVER['DOCUMENT_ROOT']."/sale_ajax_arResult.txt", print_r($arResult, true));
	...
	<script>
        ...
        BX.addCustomEvent('onAjaxSuccess',function(){
            if($("#soa-property-5").length)
            {
                maskAktive("",true,"#soa-property-5");
            }
            
            if($("#soa-property-22").length)
            {
                $("#soa-property-22").inputmask('dd.mm.yyyy',{ "placeholder": "__.__.____" });
            }
        });
        // закомментил в связи с задачей № 14781
