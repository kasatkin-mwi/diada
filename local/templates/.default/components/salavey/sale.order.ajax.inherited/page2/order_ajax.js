...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
			...
			if (action == 'saveOrderAjax')
			{
				...
				BX.ajax.submitAjax(
					BX('bx-soa-order-form'),
					{
						...
						onsuccess: BX.proxy(this.saveOrder, this),
				...
				switch (k.toUpperCase())
				{
					...
					case 'DELIVERY':
						if ( sdec ){
							...
							$('#bx-soa-delivery').find('.alert-danger').text( blockErrors ).show();
						}
						else {
		...
		editActiveDeliveryBlock: function(activeNodeMode)
		{
			...
			this.showDeliveryDescription(activeNodeMode);
		},

		showDeliveryDescription: function(activeNodeMode)
		{
			
			/*
			var deliveryBlockDescription = BX('delivery_description_always_seen');
			
			if(activeNodeMode === false && deliveryBlockDescription)
				BX.remove(deliveryBlockDescription);
			
			if(activeNodeMode === true)
			{
				if( !deliveryBlockDescription )
				{
					BX.insertAfter(
						BX.create('DIV', {
		...
		editFadeDeliveryBlock: function()
		{
			...
			this.showDeliveryDescription(false);//скрыть блок описание доставки
			if (this.initialized.delivery)
			{
		...
		{
			if(item.PRICE > 0) {
				item.PRICE_FORMATED = item.PRICE + ' р.';			
