...
<? if ( $request->get('ORDER_ID') ): ?>
    <div class="standart_width" style="padding: 40px 0;margin-bottom: 20px;">
        ...
        <?
		$arResult["ORDER"]["ID"] = $request->get('ORDER_ID');
        ...
        <div>
            <div class="basket_page_left_col">
                <div class="new_basket_el order_create_success">
                    <div>Ваш заказ №<?=$order->getField("ACCOUNT_NUMBER")?> оформлен!</div>
                    <?
                ...
                <div class="order_create_success_title">Номер заказа <?=$order->getField("ACCOUNT_NUMBER")?></div>
					...
					<div class="new_basket_el order_info_success">
									...
									<div class="block_subsection_column">
										<div class="left_block">
											...
											<?if (preg_match("/Доставка|доставка/",$shipment->GetField("DELIVERY_NAME"))):?>
												<?
                                                if(!empty($listProps[68]))
                                                {
                                                    ?>
                                                    <p>
                                                        По адресу: <?=$listProps[68]?>
                                                    </p>
                                                    <?
                                                }
                                                else
						...
						<div class="block_subsection block_subsection_money">
							...
							<?
								$airgun = new Airgun();
								$airgun -> init();
								$airgun -> getOrderFromAir( $order->getField("ACCOUNT_NUMBER") );
								
								/*
								CSaleOrder::PayOrder($arResult["ORDER"]["ID"], "Y");
								...
								$paySystemService = \Bitrix\Sale\PaySystem\Manager::getObjectById($onePayment->getPaymentSystemId());
								$initResult = $paySystemService->initiatePay($onePayment, null, \Bitrix\Sale\PaySystem\BaseServiceHandler::STRING);
								if ($initResult->isSuccess())
									echo $arPaySysAction['BUFFERED_OUTPUT'] = $initResult->getTemplate();
								else
