									...
									<div class="new_basket_el_left_title">
                                        ...
                                        <div class="block_props_basket">
                                            <?foreach ($arItem["PROPS"] as $infoProp):
                                                if($infoProp["CODE"] == "SERVICE")
                                                    continue;
                                            ?>
                                                <?if ((!preg_match("/^HIDE_/i",$infoProp["CODE"]) && ($infoProp["CODE"] != 'ITEM_CODE'))):?>
                                                    <span><?=$infoProp["NAME"]?>:</span> <?=$infoProp["VALUE"]?><br />
                                                <?elseif ($infoProp["CODE"] == 'ITEM_CODE'):?>
                                                	<span><?=$infoProp["NAME"]?>:</span> <?=$infoProp["VALUE"]?><br />
                                                <?else:?>
							...
							?>
										...
										<div class="old_price" id="old_price_<?=$arItem["ID"]?>">
                                            <?if (!empty($dataProp["HIDE_OLD_PRICE"]) && (floatval($dataProp["HIDE_OLD_PRICE"]["VALUE"]) > floatval($arItem["PRICE"]))):?>
                                                <?                                                
                                                    $arOldPrice = $arOldPrice + ($dataProp["HIDE_OLD_PRICE"]["VALUE"] * $arItem["QUANTITY"]);
                                                    $showOldPrice = true;
                                                ?>
                                            ...
                                            <?else:?>
                                                <?if (floatval($arItem["BASE_PRICE"]) > floatval($arItem["PRICE"])):?>
                                                    <?                                                    
                                                        $arOldPrice = $arOldPrice + ($arItem["FULL_PRICE"] * $arItem["QUANTITY"]);
                                                        $showOldPrice = true;
                                                    ?>
                                                    <s><?=$arItem["FULL_PRICE_FORMATED"]?></s>
                                                <?else:?>
                                                    <?                                                    
                                                        $arOldPrice = $arOldPrice + ($arItem["PRICE"] * $arItem["QUANTITY"]);
                                                    ?>
                                                <?endif;?>
                                            <?endif;?>
										</div>
			...
			<div class="basket_page_right_col">
				<div class="basket_page_itog_block js_new_form_lk_gray_bg default">
                    ...
                    <div class="basket_page_itog_el">
						...
						<div class="basket_page_itog_el_param" id="allSum_FORMATED">
                            <?=str_replace(" ", "&nbsp;", $arResult["allSum_FORMATED"])?>
                            <?if (floatval($arOldPrice) > 0 && $showOldPrice):?>
                                <br>
                                <span class="bx-price-old">
                                    <?=SaleFormatCurrency($arOldPrice, $arResult["CURRENCY"])?>
                                </span>  
                            <?endif;?>
                        </div> 
					</div>
					...
					<?/* <a href="javascript:;" onclick="checkOut();" class="checkout red_button fancy_pay_war"><?=GetMessage("SALE_ORDER")?></a>*/?>
	...
	$(document).ready(function() {
		$("body").on("click", "#coupon_button", function() {			
			if ($("#coupon").val().length > 0) {
				...
				$.ajax ({
					url: "/personal/cart/ajax_promo.php",
					type: "get",
					data: {coupon: coupon},
		...
		$("body").on("keypress", function(e) {			
			if (($("#coupon").val().length > 0) && (e.keyCode==13)) {
				var coupon = $("#coupon").val();
				$.ajax ({
					url: "/personal/cart/ajax_promo.php",
					type: "get",
		...
		$("body").on("click", "#close_coupon", function() {		
			...
			$.ajax ({
				url: "/personal/cart/ajax_promo.php",
				type: "get",
				data: {coupon_clear: coupon_clear},
				success: function(html){
					window.location.replace("");
				}
			})
