...
<div class="bx-auth-serv-icons">
	...
	?>
	<a title="<?=htmlspecialcharsbx($service["NAME"])?>" href="javascript:void(0)" <?=$onClickEvent?> id="bx_auth_href_<?=$arParams["SUFFIX"]?><?=$service["ID"]?>"><span class="i bx-ss-icon <?=htmlspecialcharsbx($service["ICON"])?>"></span></a>
	<?endforeach?>
</div>
<?endif;?>
<div class="bx-auth">
	<form method="post" name="bx_auth_services<?=$arParams["SUFFIX"]?>" target="_top" action="<?=$arParams["AUTH_URL"]?>">
		...
		<?if($arParams["~FOR_SPLIT"] != 'Y'):?>
			<div class="bx-auth-services">
				<?foreach($arAuthServices as $service):?>
					<div><a href="javascript:void(0)" onclick="BxShowAuthService('<?=$service["ID"]?>', '<?=$arParams["SUFFIX"]?>')" id="bx_auth_href_<?=$arParams["SUFFIX"]?><?=$service["ID"]?>"><span class="i bx-ss-icon <?=htmlspecialcharsbx($service["ICON"])?>"></span><span class="strong"><?=htmlspecialcharsbx($service["NAME"])?></span></a></div>
				<?endforeach?>
			</div>
		<?endif;?>
