...
if (isset($arResult['ITEM']))
{
    ...
    if ($item['~PREVIEW_PICTURE']>0) {
        $resizeImg = CFile::ResizeImageGet($item['~PREVIEW_PICTURE'], array("width" => 226, "height" => 130))["src"];
    }
    elseif ($item['~DETAIL_PICTURE']>0){
        $resizeImg = CFile::ResizeImageGet($item['~DETAIL_PICTURE'], array("width" => 226, "height" => 130))["src"];
    }
    if (strlen($resizeImg) == 0){
        $resizeImg = $resizeImgNoPhoto;
    }
	?>
    <!-- <div class="index_produce index_produce_in4" style="position: relative;" id="<?=$areaId?>" data-entity="item"> -->
    <div class="newcat_el_bl" id="<?=$areaId?>" data-entity="item">
		<?
