...
if(isset($arResult["DB_SOCSERV_USER"]) && $arParams["SHOW_PROFILES"] != 'N')
{
	...
	<div class="soc-serv-accounts soc-serv-accounts_list">
		<?/*<table cellspacing="0" cellpadding="8">
			...
			foreach($arResult["DB_SOCSERV_USER"] as $key => $arUser)
			{
				...
				?>
						<div class="lk_soc_el">
							<div class="lk_soc_el_icon">
								<i class="bx-ss-icon <?=$icon?>">&nbsp;</i>
							</div>
							<div class="lk_soc_el_txt">
								<div><?if ($arUser["PERSONAL_LINK"] != ''):?>
								...
								<?endif;?></div>
								<div><?if (in_array($arUser["ID"], $arResult["ALLOW_DELETE_ID"])):?>
									<a class="split-delete-item" href="?action=delete&user_id=<?=$arUser["ID"]."&".bitrix_sessid_get()?>" onclick="return confirm('<?=GetMessage("SS_PROFILE_DELETE_CONFIRM")?>')" title=<?=GetMessage("SS_DELETE")?>><?=GetMessage("SS_DELETE")?></a>
								<?endif;?></div>
							</div>
						</div>
...
{
	?>
	<div class="soc-serv-title-grey_bl">
		<div class="soc-serv-title-grey">
