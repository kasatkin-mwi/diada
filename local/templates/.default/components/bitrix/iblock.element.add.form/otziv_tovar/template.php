...
<form name="iblock_add" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data">
	...
	<div class="new_call_back_light_title">Оставить отзыв о товаре</div>
	<div class="new_call_back_light_text"></div>
		<?if (is_array($arResult["PROPERTY_LIST"]) && !empty($arResult["PROPERTY_LIST"])):?>

			<?foreach ($arResult["PROPERTY_LIST"] as $propertyID):?>
				<?if ($arResult["PROPERTY_LIST_FULL"][$propertyID]['CODE'] == 'TOVAR'):?>
					<input type="hidden" name="PROPERTY[<?=$propertyID?>][0]" value="<?=$arParams['PRODUCT_ID']?>" />
					<?continue;?>
				<?endif;?>
				<ul class="new_call_back_light_form">
					<li><?if (intval($propertyID) > 0):?><?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"]?><?else:?><?=!empty($arParams["CUSTOM_TITLE_".$propertyID]) ? $arParams["CUSTOM_TITLE_".$propertyID] : GetMessage("IBLOCK_FIELD_".$propertyID)?><?endif?><?if(in_array($propertyID, $arResult["PROPERTY_REQUIRED"])):?><span class="starrequired">*</span><?endif?></li>
					<li>
						<?
						...
						switch ($INPUT_TYPE):
							...
							case "T":
								...
								?>
						<textarea placeholder="Укажите максимально подробную информацию" cols="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>" rows="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]"><?=$value?></textarea>
								<?
								}
							break;
					...
					[<a href="<?=$arResult["ELEMENT_FILES"][$value]["SRC"]?>"><?=GetMessage("IBLOCK_FORM_FILE_DOWNLOAD")?></a>]<br />
							...
							<input type="<?=$type?>" name="PROPERTY[<?=$propertyID?>]<?=$type == "checkbox" ? "[".$key."]" : ""?>" value="<?=$key?>" id="property_<?=$key?>"<?=$checked ? " checked=\"checked\"" : ""?> /><label for="property_<?=$key?>"><?=$arEnum["VALUE"]?></label><br />
									...
									case "multiselect":
									?>
                                    <?if ($arResult["PROPERTY_LIST_FULL"][$propertyID]['CODE'] == 'OCENKA'):?>
                                    	<div class="add_comment_star">
                                    	<?foreach ($arResult["PROPERTY_LIST_FULL"][$propertyID]["ENUM"] as $key => $arEnum):?>
                                    		<input type="radio" name="PROPERTY[<?=$propertyID?>]" value="<?=$key?>" />
                                    	<?endforeach;?>
                                    	</div>
                                    <?else:?>
...
<div id="g-recaptcha" style="margin-bottom: 15px; float: right"></div> -->
		<?endif?>
		<div class="new_call_back_light_right_button">
			<input class="red_button" type="submit" name="iblock_submit" value="<?=GetMessage("IBLOCK_FORM_SUBMIT")?>" />
		</div>
</form>
