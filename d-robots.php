...
class RobotsTxtParser
{
		...
		foreach (array(self::DIRECTIVE_DISALLOW, self::DIRECTIVE_ALLOW) as $directive) {
			if (isset($this->rules[$userAgent][$directive])) {
				foreach ($this->rules[$userAgent][$directive] as $robotRule) {
					// check rule
					//var_dump($robotRule, $this->checkRuleSwitch($robotRule, $path));
	...
	{
		$ruleEncoded = $this->encode_url($rule);
		// change @ to \@
		$escaped = strtr($rule, array("@" => '\@'));
		$escapedEncoded = strtr($ruleEncoded, array("@" => '\@'));
		// match result
			
		if (preg_match('@' . $escaped . '@', $path) || preg_match('@' . $escapedEncoded . '@', $path)) {
