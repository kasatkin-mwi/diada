...
catch(err){if(!gData[fname]){var s=document.createElement('script');s.src=path;document.head.appendChild(s);gData[fname]=!0}
setTimeout(function(){self.run(path,func,fname)},1000)}}
self.getForm=function(){self.sended=!1;var datajax;datajax="formid="+_id+'&route=DSFormView';if(self.config){var configs=self.config.replace(/'/g,'"');try{eval(JSON.parse(configs));datajax+="&dsconfig="+configs}
catch(e){console.error('JSON array is improper for #'+_id+'!')}}
$.ajax({url:root+"index.php",type:"POST",dataType:"text",data:datajax,cache:!1,success:function(data){__construct__(data)}})}
...
else if(_o.get(0).dsformmarker===!0){console.warn('DSFORM ('+options.formID+'): element already uses \n Path: '+__path__(self.element)+'\n Element:',self.element.get(0))}
