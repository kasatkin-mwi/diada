...
class DSFormSend extends DSMain {
	...
	public function index() {
		...
		if (!$this->formConfig->getConfig($this->formID)) 
            throw new Exception("file config form", 1);

			foreach ($this->formConfig->formFields as $field) 
            {
				if(isset($field['attributs']['required'])) 
                {
					if(!isset($field['attributs']['pattern'])) 
                        $field['attributs']['pattern'] = '';
            ...
            if(empty($this->error))
            {
                foreach ($this->formConfig->formFields as $field) 
                {
                    if(isset($field['attributs']['required']) && $field['type'] == 'recaptcha') 
                    {
                        $captchaResponse = $this->post[$field['attributs']['name']];
                        if($captchaResponse)
                        {           
                            if(\GoogleReCaptcha::checkClientResponse())
                            {
                                $this->error[$field['attributs']['name']] = $field['error'];
                            }      
                        }
                        else
                        {
		...
		if(@$this->mail->send()){
            ...
            if($this->formConfig->make_call == 'Y')
            {        
                ...
                foreach ($this->formConfig->formFields as $field) 
                {
                    if(isset($field['attributs']['name']))
                    {
                        $phonenumber = $this->post[$field['attributs']['name']];
                ...
                if(!empty($phonenumber))
                {
                    ...
                    $tel_call->make_call($source, $destination, $phonenumber);    
                }
            }
		//if(true){
			$this->responseJson(
