...
{
    $rsEl = CIBlockElement::GetList(array("ID" => "ASC"), array("IBLOCK_ID" => $BID, ">ID" => $_REQUEST["lastid"]), false, array("nTopCount" => $limit), array(
        'ID',
        'IBLOCK_ID',
        'CATALOG_STORE_AMOUNT_3'
    ));
    while ($arEl = $rsEl->Fetch())
    {
        $arFields = array(
            'STORE_ID' => 3,
            'PRODUCT_ID' => $arEl['ID'],
            'AMOUNT' => $arEl['CATALOG_STORE_AMOUNT_3'],
        );
        //echo '<pre>'; echo '<br>'; var_export($arFields); echo '</pre>'; die;
        Salavey::SetProductIndicator(false, $arFields);
