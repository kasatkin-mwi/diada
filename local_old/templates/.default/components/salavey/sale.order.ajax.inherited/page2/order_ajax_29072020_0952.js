...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		FullOrderPrice: 0,
        ListBasketOrderPriceAdded: [],
		bAlreadyShowBoxBerryMap: false,
		/**
			...
			for (k in errors)
			{
				...
				switch (k.toUpperCase())
				{
					case 'MAIN':
						...
						this.animateScrollTo(this.mainErrorsNode, 800, 20);
                        var button = $(this.mainErrorsNode).find(".bx-soa-editstep");
						scroll = false;
						break;
					case 'AUTH':
		...
		showDeliveryItemsPage: function(page)
		{
			...
			var _deliveries = document.getElementsByClassName('bx-soa-pp-company-checkbox');			
		},
		...
		bindEvents: function()
		{
            BX.bind(this.orderSaveBlockNode.querySelector('a.btn'), 'click', BX.proxy(this.clickOrderSaveAction, this));
			...
			/* BX.addCustomEvent('onAjaxSuccess', BX.proxy(this.showDeliveryDescription, this)); */
		},
		
		/* showDeliveryDescription: function()
		{
			console.log('showDeliveryDescription',this.result);
			//this.showBlockWarning(this.deliveryBlockNode, 'fdsgsdgdsgds', false);
		}, */
		
		ShowBoxBerryMap: function()
		{
			var currentDelivery = this.getSelectedDelivery();
			if((currentDelivery.ID == 204 || currentDelivery.ID == 206) && !this.bAlreadyShowBoxBerryMap)
			{
				this.bAlreadyShowBoxBerryMap = true;
				var re = /^.*?onclick="(.*?)".*?$/;
				var code = currentDelivery.PERIOD_TEXT.replace(re, '$1').replace('return false;','');
				
				if(code!='') eval(code);
			}
			else
			{
				this.bAlreadyShowBoxBerryMap = false;
			}
		},
		...
		getBlockFooter: function(node)
		{
			...
			if (!isLastNode)
			{
                if ($(currentSection)[0].id == "bx-soa-basket") {
                    buttons.push(
                        BX.create('A', {
                            props: {href: '/personal/cart/', className: 'pull-left btn btn-default btn-md'},
                            text: this.params.MESS_TO_BASCET
                        })
                    );
                }
                buttons.push(
		...
		createBasketItemColumn: function(column, allData, active)
		{
			...
			{       
				...
				if (parseFloat(data.BASE_PRICE) > parseFloat(data.PRICE))
				{
			...
			else if (column.id == 'SUM')
			{
                ...
                var PriceSet = false;
                for (var i in props){
                	codeProp = props[i].CODE;
                	if (codeProp == "HIDE_OLD_PRICE"){
                        SetPrice = props[i].VALUE*parseFloat(data.QUANTITY);
                        if (!PriceSet && this.ListBasketOrderPriceAdded.indexOf(data.ID) == -1) {
                            this.ListBasketOrderPriceAdded.push(data.ID);
                            this.FullOrderPrice += SetPrice;
                            PriceSet = true;
                        }
					}
				}
				if (!PriceSet){
                    if (this.ListBasketOrderPriceAdded.indexOf(data.ID) == -1) {
                        this.ListBasketOrderPriceAdded.push(data.ID);
                        this.FullOrderPrice += parseFloat(data.SUM_NUM);
                        PriceSet = true;
                    }
				}
				textNode.appendChild(BX.create('STRONG', {props: {className: 'bx-price all'}, html: data.SUM}));
				if (!SetPrice && (parseFloat(data.BASE_PRICE) > parseFloat(data.PRICE)))
				{
					textNode.appendChild(BX.create('BR'));
					textNode.appendChild(BX.create('STRONG', {
						props: {className: 'bx-price-old'},
				...
				else if(SetPrice && (parseFloat(SetPrice) > parseFloat(data.PRICE))){
                    textNode.appendChild(BX.create('BR'));
                    textNode.appendChild(BX.create('STRONG', {
                        props: {className: 'bx-price-old'},
                        html: SetPrice+" р."
		...
		editActiveDeliveryBlock: function(activeNodeMode)
		{
			...
			var visited = this.deliveryBlockNode.getAttribute('data-visited') == 'true';
				console.log('visited',visited);
				console.log('selected',BX.hasClass(this.deliveryBlockNode,'bx-selected'));
		...
		editDeliveryInfo: function(deliveryNode)
		{
			...
			if(! BX('delivery_description_always_seen') )
			{
				var visited = this.deliveryBlockNode.getAttribute('data-visited') == 'true';
				/* console.log('visited',visited);
				console.log('selected',BX.hasClass(this.deliveryBlockNode,'bx-selected')); */
				if(
					BX.hasClass(this.deliveryBlockNode,'bx-selected') || visited	
				)
				{
					BX.insertAfter(
						BX.create('DIV', {
							attrs: {'class': 'order_red_txt', 'id': 'delivery_description_always_seen'},
							text: 'Стоимость доставки может отличаться в зависимости от габаритов и веса груза, она будет рассчитана менеджером при подтверждении заказа.'
						}),this.deliveryBlockNode.querySelector('.bx-soa-section-title-container')
					);
				}
			}
			/* if(! BX('delivery_description_always_seen') && 
				BX.hasClass(this.deliveryBlockNode,'bx-selected') &&
				BX.hasClass(this.deliveryBlockNode,'bx-active')
			)
			{
			...
			//var deliveryTitle = this.deliveryBlockNode.querySelector('.bx-soa-section-title-container');
			/* console.log('editActiveDeliveryBlock',this.deliveryBlockNode);
			if(
				
				BX.hasClass(this.deliveryBlockNode,'bx-selected')
			)
			{
				console.log(this.deliveryBlockNode,this.deliveryBlockNode.getAttribute('class'));
			} */
			
			/* BX.prepend(BX.create('DIV', {
					props: {className: 'order_red_txt'},
					text: 'Стоимость доставки может отличаться в зависимости от габаритов и веса груза, она будет рассчитана менеджером при подтверждении заказа.'
				}), deliveryContent); */
			
			/* BX.insertAfter(
				BX.create('DIV', {
					attrs: {className: 'order_red_txt', id: 'delivery_description_always_seen'},
					text: 'Стоимость доставки может отличаться в зависимости от габаритов и веса груза, она будет рассчитана менеджером при подтверждении заказа.'
				}),deliveryTitle
			); */
			...
			if (currentDelivery.PRICE >= 0)
			{
				...
				/* if((currentDelivery.ID==204 || currentDelivery.ID==206) && currentDelivery.CHECKED=='Y')
				{
					boxberry.checkLocation(1);boxberry.open(delivery, '1$vUXV7BDt8pxTBD0HMCCoMblmtgcbbh55' , ' МОСКВА ' , '', '15780' , '1000' , '0' ,'0','0','0' ); return false;
				} */
			}
		...
		{
			if(item.PRICE > 0) {
				item.PRICE_FORMATED = 'от ' + item.PRICE + ' р.';			
				item.DELIVERY_DISCOUNT_PRICE_FORMATED = item.PRICE_FORMATED;
			}
		...
		createPickUpItem: function(currentStore, options)
		{
			...
			var imgPicUpSet = "";
			$.ajax({
				url: "/include/get_img_store.php",
				data: {STORE_ID: currentStore.ID},
                async: false,
				dataType: "json",
				success: function (data) {
                    imgPicUpSet = data.LOGOTIP_METRO;
                }
			});
			...
			var imageAddStore = "";
			if (imgPicUpSet){
				imageAddStore = BX.create('img', {
					props: {className: 'logo_metro'},
					attrs: {"src": imgPicUpSet},
		...
		getPropertyRowNode: function(property, propsItemsContainer, disabled)
		{
			...
			var propsItemNode = BX.create('DIV'),
				...
				label;
            var selectedPersonType = this.getSelectedPersonType(),props = [],fadeParamName,i;
			if (disabled)
			{
		...
		editTotalBlock: function()
		{
            ...
            if (!OldShow && this.FullOrderPrice>0) {
                priceHtml += '<br /><span class="bx-price-old">' + this.FullOrderPrice + ' р.</span>';
            }
            if (!this.result.SHOW_AUTH)
            {
                this.totalInfoBlockNode.appendChild(
                    BX.create('DIV', {
                        ...
                        children: [
                            ...
                            BX.create('p', {
                                props: {
                                    className: 'personaldatamob'
                                },
                                html: "Нажимая кнопку \"<b>ОФОРМИТЬ ЗАКАЗ</b>\", я подтверждаю свою дееспособность, согласие на получение информации об оформлении и получении заказа, согласие на обработку персональных данных в соответствии с указанным <a href=\"javascript:void(0)\" class=\"showuserconset\" style=\"color: #a41618\"><b>здесь</b></a> текстом.",
                            })
                        ]
                    })
                );
