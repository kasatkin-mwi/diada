	...
	<div class="bx-filter-section container-fluid">
		<form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get" class="smartfilter">
			...
			<div class="row">
				<?foreach($arResult["ITEMS"] as $key=>$arItem)//prices
				{
					...
					if(isset($arItem["PRICE"])):
						...
						<div class="element_filter_block bx-filter-parameters-box bx-active">
							...
							<div class="bx-filter-parameters-box-title" onclick="smartFilter.hideFilterProps(this)"><span><?=(preg_match("/базовый/",$arItem["NAME"]))?"Цена, руб.":$arItem["NAME"]?> <span data-role="prop_angle" class="fa fa-angle-<?if ($arItem["DISPLAY_EXPANDED"]== "Y"):?>up<?else:?>down<?endif?>"></span></span></div>
				...
				{
                    if (isset($arParams["SHOW_THIS_LIST"]) && is_array($arParams["SHOW_THIS_LIST"]) && count($arParams["SHOW_THIS_LIST"])>0) {
                        if (!in_array($arItem["ID"], $arParams["SHOW_THIS_LIST"])) continue;
                    }
					if(
						empty($arItem["VALUES"])
						|| isset($arItem["PRICE"])
					...
					if (
						...
						continue;
					?>
					<div class="element_filter_block bx-filter-parameters-box <?if ($arItem["DISPLAY_EXPANDED"]== "Y"):?>bx-active<?endif?>">
						<span class="bx-filter-container-modef"></span>
						...
						<div class="filter_light js_filter_light bx-filter-block" data-role="bx_filter_block">
							...
							switch ($arItem["DISPLAY_TYPE"])
							{
								case "A"://NUMBERS_WITH_SLIDER
									...
									?>
                                    <?
                                    if($USER->IsAdmin() && $_SERVER['REMOTE_ADDR'] == '87.76.253.206')
                                    {
                                       // echo '<pre>'; echo '<br>'; var_export($arJsParams); echo '</pre>';    
                                    }
                                    ?>
									<script type="text/javascript">
										BX.ready(function(){
										...
										<label class="bx-filter-param-label" for="<? echo "all_".$arCur["CONTROL_ID"] ?>">
											<span class="bx-filter-input-checkbox">
												<input
													...
													id="<? echo "all_".$arCur["CONTROL_ID"] ?>"
													onsubmit="smartFilter.click(this)"
												/>
										...
										<div class="radio">
											<label data-role="label_<?=$ar["CONTROL_ID"]?>" class="bx-filter-param-label" for="<? echo $ar["CONTROL_ID"] ?>">
												<span class="bx-filter-input-checkbox <? echo $ar["DISABLED"] ? 'disabled': '' ?>">
													<input
                                                        onchange="smartFilter.click(this)" data-role="label_<?=$ar["CONTROL_ID"]?>"
														type="radio"
														value="<? echo $ar["HTML_VALUE_ALT"] ?>"
