...
{
	?><div class="error_page_nothelp_txt js_error_page_nothelp_txt">
		 Поделом Арчи! ) Пусть оттачивает поисковые навыки! Воспользуйтесь строкой "Умного поиска" или позвоните нам по бесплатному номеру <a href="tel:88003330742">8-800-333-07-42</a> <br>
		 У нас более 10 000 товаров, мы обязательно подберем то, что Вы давно искали!
	</div>
	<script>
...
<?if(empty($arParams["REQUIRED_FIELDS"]) || in_array("MESSAGE", $arParams["REQUIRED_FIELDS"])):?><?endif?>
		...
		<textarea type="hidden" style="display:none" name="MESSAGE" rows="5" cols="40">Не помогать! Пусть сам ищет!</textarea>
	</div>
 ...
 <input class="error_page_gray_bt js_error_page_gray_bt" type="submit" name="submit" value="Не помогать! Пусть сам ищет!">
