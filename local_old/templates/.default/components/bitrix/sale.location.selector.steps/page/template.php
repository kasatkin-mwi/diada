...
<?else:?>
	...
	<div id="sls-<?=$arResult['RANDOM_TAG']?>" class="bx-slst<?if($arResult['ADMIN_MODE']):?> bx-admin-mode<?endif?>">
		...
		<?if(is_array($arResult['DEFAULT_LOCATIONS']) && !empty($arResult['DEFAULT_LOCATIONS'])):?>
			<p class="primert_local quick-locations">
                Например,
    ...
    <?
    if(empty($arResult['LOCATION']['ID']))
    {
        $arResult['LOCATION']['ID'] = 2541;
        echo '<input type="hidden" id="LOCATION_ALT_PROP_DISPLAY_MANUAL[17]" name="LOCATION_ALT_PROP_DISPLAY_MANUAL[17]" value="1" />';   
    }
    ?>
