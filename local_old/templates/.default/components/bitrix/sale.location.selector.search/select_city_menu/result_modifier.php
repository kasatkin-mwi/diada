...
$resTown = CIBlockElement::GetList(array("SORT" => "ASC"),array("IBLOCK_ID" => 20,"PROPERTY_SHOW_MENU" => 995, "NAME" => array("Санкт-Петербург","Москва")));
while($objtown = $resTown->GetNextElement()){
    $infoTown = array_merge($objtown->GetFields(),array("PROP" => $objtown->GetProperties()));
    $arResult["TOWNS_LIST"][$infoTown["PROP"]["REGION"]["VALUE"]] = $infoTown["NAME"];
}
$resTown = CIBlockElement::GetList(array("NAME" => "ASC"),array("IBLOCK_ID" => 20,"PROPERTY_SHOW_MENU" => 995, "!NAME" => array("Санкт-Петербург","Москва")));
while($objtown = $resTown->GetNextElement()){
	$infoTown = array_merge($objtown->GetFields(),array("PROP" => $objtown->GetProperties()));
	$arResult["TOWNS_LIST"][$infoTown["PROP"]["REGION"]["VALUE"]] = $infoTown["NAME"];
}
